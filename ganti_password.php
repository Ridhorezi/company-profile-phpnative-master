...
if($token == '' or $email == ''){
    $err .= "Invalid link. Email and token not available.";
}else{
    ...
    if($n1 < 1){
        $err .= "Invalid link. Email and token do not match";
    }
}
...
if(isset($_POST['submit'])){
    ...
    if($password == '' or $konfirmasi_password == ''){
        $err .= "Please enter the password and confirm the password";
    }elseif($konfirmasi_password != $password){
        $err .= "Confirm password does not match the password";
    }elseif(strlen($password)<6){
        $err .= "The number of characters allowed for a password is at least 6 characters";
    }

    if(empty($err)){
        $sql1 = "update members set token_ganti_password = '',password=md5($password) where email = '$email'";
        mysqli_query($koneksi,$sql1);
        $sukses = "Password has been changed successfully. Please <a href='".url_dasar()."/login.php'>login</a>.";
    }
}
?>
